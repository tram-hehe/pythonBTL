...
class EmployeeManagementApp:
    ...
    def create_widgets(self):
        ...
        tk.Button(frame_buttons, text="Xem doanh thu", command=self.show_employee_sales).pack(side=tk.LEFT, padx=5)

        self.tree = ttk.Treeview(self.root, columns=(
            "Mã nhân viên", "Họ và tên", "Giới tính", "Địa chỉ", "Số điện thoại", "Chức vụ", "Doanh thu"),
                                 show='headings')

        for label in self.tree['columns']:
            self.tree.heading(label, text=label)
            self.tree.column(label, width=120)  # Cột rộng hơn để dễ nhìn
    ...
    def show_employee_sales(self):
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT e.name, COALESCE(SUM(s.amount), 0) AS total_sales
            FROM employees e
            LEFT JOIN sales s ON e.employee_id = s.employee_id
            GROUP BY e.name
        """)
